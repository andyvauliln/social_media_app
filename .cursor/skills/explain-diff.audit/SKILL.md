...
description: Generate a short semantic explanation of code changes from a Claude turn audit. Shows what logic/data/signatures changed in plain English. Use composer2 model only.
model: composer2
...
Also check `tools_summary` to detect creates vs edits vs deletes.
...
- If old_preview is empty → it's a new addition; if new_preview is empty → deletion
