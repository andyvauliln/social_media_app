...
#
# This helper runs: cd <agents/…/subdir> && agent --workspace <git-repo-root> [your-args…]
# Same flags as repo-root `ragent.cursor.sh`; **cwd** is the agent folder (parity with
# `cd AGENT_DIR && claude …` in `ragent.claude.sh`) so relative paths and pwd-sensitive tools
# match that agent’s directory.
#
...
# --workspace <path>
#   Repository root for discovering `.cursor`, rules, MCP — **this script pins it to the git root**.
# -w | --worktree [name]
...
ROOT_DIR="$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel 2>/dev/null || printf '%s\n' "${SCRIPT_DIR}")"
AGENT_DIR="${1:?agent dir}"
shift
